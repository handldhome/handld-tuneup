 ...
 */
/**
 * Clean up a single block of text (e.g. general notes).
 */
export async function cleanUpGeneralNotes(text) {
  ...
  try {
    ...
    const message = await client.messages.create({
      ...
      messages: [
        {
          role: 'user',
          content: `You are editing technician notes for a home inspection report that will be sent to a homeowner. Clean up this text to be professional, clear, and concise. Keep the same meaning and technical details — just improve grammar, spelling, and tone. Write in third person. Keep it to 2-3 sentences max. Do not add information that wasn't in the original note.

Return ONLY the cleaned text. No quotes, no labels, no other text.
...
${text}`,
    ...
    console.error('[CleanNotes] Error cleaning general notes, using original:', err.message)
    return text
  }
}
