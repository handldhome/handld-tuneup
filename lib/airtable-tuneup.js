...
export async function getReport(reportId) {
  ...
  return {
    ...
    sectionNotes: {
      'Exterior': data.fields['Section Notes: Exterior & Curb Appeal'] || '',
      'Electrical': data.fields['Section Notes: Electrical Systems'] || '',
      'Plumbing': data.fields['Section Notes: Plumbing & Water Systems'] || '',
...
export async function getTaskResults(reportId) {
  ...
  // Use the linked record field name with SEARCH function
  const filterFormula = `SEARCH("${reportId}", ARRAYJOIN({TuneUp Reports}))`;
