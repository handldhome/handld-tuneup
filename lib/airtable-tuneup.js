...
export async function getTaskResults(reportId) {
  ...
  try {
    // Use filterByFormula to get tasks for this specific report
    const formula = encodeURIComponent(`FIND("${reportId}", ARRAYJOIN({TuneUp Reports}))`);
    const url = `/${tableName}?filterByFormula=${formula}&sort%5B0%5D%5Bfield%5D=Task%20Number&sort%5B0%5D%5Bdirection%5D=asc`;

    console.log('[TuneUp] Fetching tasks with filter for report:', reportId);

    const data = await airtableRequest(url);

    console.log('[TuneUp] Found', data.records.length, 'task results for report', reportId);
    ...
    // Fallback to client-side filtering if Airtable formula fails
    console.log('[TuneUp] Falling back to client-side filtering');
    ...
    const filteredRecords = data.records.filter(record => {
      ...
      const isMatch = linkedReports.includes(reportId);
      if (linkedReports.length > 0) {
        console.log('[TuneUp] Task record linked to:', linkedReports, 'checking against:', reportId, 'match:', isMatch);
      }
      return isMatch;
    });

    console.log('[TuneUp] Found', filteredRecords.length, 'task results after filtering');
