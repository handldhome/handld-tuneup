...
export async function getTaskResults(reportId) {
  ...
  try {
    ...
    // Use SEARCH to find the reportId in the linked records array
    const formula = `SEARCH("${reportId}", ARRAYJOIN({TuneUp Reports}, ","))`;
...
export async function uploadPhotoToTask(reportId, taskNumber, photo) {
  ...
  try {
    ...
    const formula = `AND(SEARCH("${reportId}", ARRAYJOIN({TuneUp Reports}, ",")), {Task Number} = ${taskNumber})`;
    const encodedFormula = encodeURIComponent(formula);
