    ...
    try {
      do {
        ...
        if (!data || !data.records) {
          console.error('[TuneUp] Invalid response from Airtable:', data);
      ...
      console.error('[TuneUp] Error during pagination:', paginationError);
      console.error('[TuneUp] Pagination state - pageCount:', pageCount, 'offset:', offset, 'records so far:', allRecords.length);
      throw new Error(`Failed to fetch task records: ${paginationError.message}`);
    }
