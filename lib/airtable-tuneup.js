...
export async function getTaskResults(reportId) {
  ...
  try {
    ...
    // Try converting the linked field to string directly instead of ARRAYJOIN
    // This might work better with Airtable's linked record field format
    const formula = `FIND("${reportId}", {TuneUp Reports} & "") > 0`;
