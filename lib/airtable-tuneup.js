...
export async function getTaskResults(reportId, retryCount = 0) {
  ...
  try {
    // Use the same simple approach as uploadPhotoToTask (which works!)
    // Fetch all records with simple sort, then filter client-side
    console.log('[TuneUp] Fetching task results for report:', reportId);
    ...
    do {
      pageCount++;
      // Simple query like uploadPhotoToTask - no pageSize, just sort
      const url = offset
        ? `/${tableName}?offset=${encodeURIComponent(offset)}&sort%5B0%5D%5Bfield%5D=Task%20Number&sort%5B0%5D%5Bdirection%5D=asc`
        : `/${tableName}?sort%5B0%5D%5Bfield%5D=Task%20Number&sort%5B0%5D%5Bdirection%5D=asc`;

      console.log('[TuneUp] Fetching page', pageCount);
    ...
    console.log('[TuneUp] Fetched', allRecords.length, 'total records, now filtering for report:', reportId);

    // Search ALL records to see if ANY contain our report ID
    const recordsWithOurReportId = allRecords.filter(record => {
      const linkedReports = record.fields['TuneUp Reports'] || [];
      return linkedReports.some(id => id === reportId || id.includes(reportId));
    });

    console.log('[TuneUp] Records containing report ID in dataset:', recordsWithOurReportId.length);
    if (recordsWithOurReportId.length > 0) {
      console.log('[TuneUp] First matching record:', {
        taskNumber: recordsWithOurReportId[0].fields['Task Number'],
        linkedReports: recordsWithOurReportId[0].fields['TuneUp Reports']
      });
    } else {
      console.log('[TuneUp] NO RECORDS FOUND - API hasnt returned the new tasks yet (caching issue)');
    }

    // Log first few records to see what the linked field contains
    console.log('[TuneUp] Sample of first 5 records:');
    allRecords.slice(0, 5).forEach((record, i) => {
      const linkedReports = record.fields['TuneUp Reports'] || [];
      console.log(`  Record ${i}: Task #${record.fields['Task Number']}, Linked Reports:`, linkedReports);
    });

    // Filter client-side using the same approach as uploadPhotoToTask
    const filteredRecords = allRecords.filter(record => {
      const linkedReports = record.fields['TuneUp Reports'] || [];
      const matches = linkedReports.includes(reportId);

      // Log any records that might be close matches
      if (linkedReports.length > 0 && linkedReports.some(id => id.includes('recBRN3UDVCtRokuR'))) {
        console.log('[TuneUp] POTENTIAL MATCH FOUND:', {
          taskNumber: record.fields['Task Number'],
          linkedReports: linkedReports,
          searchingFor: reportId,
          matches: matches
        });
      }
    ...
    // If no results found and we haven't exceeded retries, wait and try again (Airtable API caching issue)
    if (filteredRecords.length === 0 && retryCount < maxRetries) {
      console.log(`[TuneUp] No tasks found, retrying in ${retryDelay}ms (attempt ${retryCount + 1}/${maxRetries})`);
      await new Promise(resolve => setTimeout(resolve, retryDelay));
      return getTaskResults(reportId, retryCount + 1);
    }

    if (filteredRecords.length === 0) {
      console.log('[TuneUp] WARNING: No tasks found after', maxRetries, 'retries. Tasks may not be created yet or Airtable API cache is very stale.');
    }
