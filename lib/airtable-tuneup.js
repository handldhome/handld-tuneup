...
export async function getTaskResults(reportId) {
  ...
  // Use FIND function to search within the linked record array
  const filterFormula = `SEARCH("${reportId}", CONCATENATE({TuneUp Reports}))`;
  
  console.log('[TuneUp] Filter formula:', filterFormula);
  ...
  console.log('[TuneUp] Found', data.records.length, 'task results');
