...
export async function getReport(reportId) {
  ...
  return {
    ...
    totalGood: data.fields['Total Good'] || 0,
    totalMonitor: data.fields['Total Monitor'] || 0,
    totalRepairSoon: data.fields['Total Repair Soon'] || 0,
    totalUrgent: data.fields['Total Urgent'] || 0,
    sectionNotes: {
      'Exterior & Curb Appeal': data.fields['Section Notes: Exterior & Curb Appeal'] || '',
      'Electrical Systems': data.fields['Section Notes: Electrical Systems'] || '',
      'Kitchen & Appliances': data.fields['Section Notes: Kitchen & Appliances'] || '',
      'Plumbing & Water Systems': data.fields['Section Notes: Plumbing & Water Systems'] || '',
      'Bathrooms & Wet Areas': data.fields['Section Notes: Bathrooms & Wet Areas'] || '',
...
export async function getTaskResults(reportId) {
  const tableName = 'TuneUp Task Results';
  const filterFormula = `{Report ID} = "${reportId}"`;
  const data = await airtableRequest(
    `/${tableName}?filterByFormula=${encodeURIComponent(filterFormula)}&sort%5B0%5D%5Bfield%5D=Task%20Number&sort%5B0%5D%5Bdirection%5D=asc`
  );
