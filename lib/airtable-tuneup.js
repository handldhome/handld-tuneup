...
export async function getTaskResults(reportId) {
  ...
  try {
    ...
    allRecords.slice(0, 3).forEach((record, index) => {
      ...
      console.error(`[TuneUp] Record ${index + 1}: Task #${record.fields['Task Number']}`);
      console.error(`[TuneUp]   TuneUp Reports field:`, linkedReports);
      console.error(`[TuneUp]   Type:`, typeof linkedReports, 'Array?', Array.isArray(linkedReports));
    });
