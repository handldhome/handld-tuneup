...
export async function getTaskResults(reportId) {
  ...
  try {
    ...
    if (data.records.length > 0) {
      ...
      console.log('[TuneUp] Sample task record ID:', sampleRecord.id);
      ...
      if (sampleRecord.fields['Report ID (Lookup)']) {
        console.log('[TuneUp] Sample "Report ID (Lookup)" field:', sampleRecord.fields['Report ID (Lookup)']);
      }

      // Show full sample record
      console.log('[TuneUp] Full sample record:', JSON.stringify(sampleRecord, null, 2));
    }
