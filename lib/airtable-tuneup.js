...
export async function getTaskResults(reportId) {
  ...
  try {
    // Use filterByFormula to query only tasks for this specific report
    // This is more efficient and avoids pagination issues
    console.log('[TuneUp] Querying tasks for report:', reportId);

    // Airtable filterByFormula for linked record field
    // Use FIND to locate the reportId in the linked records array (returns position > 0 if found)
    const formula = `FIND("${reportId}", ARRAYJOIN({TuneUp Reports}, ",")) > 0`;
    const encodedFormula = encodeURIComponent(formula);

    console.error('[TuneUp] Using formula:', formula);
    console.error('[TuneUp] Encoded formula:', encodedFormula);

    const data = await airtableRequest(
      `/${tableName}?filterByFormula=${encodedFormula}&sort%5B0%5D%5Bfield%5D=Task%20Number&sort%5B0%5D%5Bdirection%5D=asc`
    );

    console.log('[TuneUp] Query returned', data.records.length, 'task results');

    if (data.records.length === 0) {
      console.error('[TuneUp] WARNING: No tasks found for report:', reportId);
      console.error('[TuneUp] This could mean:');
      console.error('[TuneUp] 1. Tasks were not properly linked to this report');
      console.error('[TuneUp] 2. The report ID is incorrect');
      console.error('[TuneUp] 3. The filterByFormula syntax is not working as expected');
    } else {
      console.log('[TuneUp] First task:', data.records[0].fields['Task Name']);
      console.log('[TuneUp] First task linked to:', data.records[0].fields['TuneUp Reports']);
    }
...
export async function uploadPhotoToTask(reportId, taskNumber, photo) {
  ...
  try {
    // Find the task result record for this report and task number using filterByFormula
    const formula = `AND(FIND("${reportId}", ARRAYJOIN({TuneUp Reports}, ",")) > 0, {Task Number} = ${taskNumber})`;
    const encodedFormula = encodeURIComponent(formula);

    console.log('[TuneUp] Searching for task with formula:', formula);

    const data = await airtableRequest(
      `/${tableName}?filterByFormula=${encodedFormula}`
    );

    console.log('[TuneUp] Found', data.records.length, 'matching task records');

    const taskRecord = data.records[0]; // Should only be one match
