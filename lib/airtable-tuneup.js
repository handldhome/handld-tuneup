...
export async function getTaskResults(reportId) {
  ...
  try {
    // Fetch ALL task results with simple pagination (fetch pages until no more data)
    ...
    do {
      pageCount++;
      const url = offset
        ? `/${tableName}?pageSize=100&offset=${encodeURIComponent(offset)}&sort%5B0%5D%5Bfield%5D=Task%20Number&sort%5B0%5D%5Bdirection%5D=asc`
        : `/${tableName}?pageSize=100&sort%5B0%5D%5Bfield%5D=Task%20Number&sort%5B0%5D%5Bdirection%5D=asc`;
      ...
      try {
        ...
        // If offset expired, break and use what we have
    ...
    // Handle both raw record ID (recXXX) and date-prefixed ID (YYYYMMDD-recXXX)
    console.error('[TuneUp] ===== FILTERING DEBUG =====');
    console.error('[TuneUp] Searching for reportId:', reportId);
    console.error('[TuneUp] Total records to filter:', allRecords.length);

    // Get all unique report IDs in the dataset
    const uniqueReportIds = new Set();
    allRecords.forEach(record => {
      const linkedReports = record.fields['TuneUp Reports'] || [];
      linkedReports.forEach(id => uniqueReportIds.add(id));
    });
    console.error('[TuneUp] All unique report IDs in dataset:', Array.from(uniqueReportIds));
    console.error('[TuneUp] Looking for:', reportId);
    console.error('[TuneUp] Is our reportId in the dataset?', uniqueReportIds.has(reportId));

    const filteredRecords = allRecords.filter(record => {
      const linkedReports = record.fields['TuneUp Reports'] || [];

      return linkedReports.some(linkedId => {
        // Direct match
        if (linkedId === reportId) return true;

        // Match if stored value has date prefix (YYYYMMDD-recXXX) but we're searching for raw ID (recXXX)
        if (linkedId.includes('-') && linkedId.split('-')[1] === reportId) return true;

        // Match if we're searching with date prefix but stored value is raw ID
    ...
    console.error('[TuneUp] Filtering complete. Found:', filteredRecords.length);
    console.error('[TuneUp] ===== END DEBUG =====');
